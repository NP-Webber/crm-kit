"use strict";

function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = PersonalDetailsForm;
var _react = _interopRequireWildcard(require("react"));
var _material = require("@mui/material");
var _EditRounded = _interopRequireDefault(require("@mui/icons-material/EditRounded"));
var _CheckRounded = _interopRequireDefault(require("@mui/icons-material/CheckRounded"));
var _CloseRounded = _interopRequireDefault(require("@mui/icons-material/CloseRounded"));
var _PersonRounded = _interopRequireDefault(require("@mui/icons-material/PersonRounded"));
var _LockRounded = _interopRequireDefault(require("@mui/icons-material/LockRounded"));
var _Visibility = _interopRequireDefault(require("@mui/icons-material/Visibility"));
var _VisibilityOff = _interopRequireDefault(require("@mui/icons-material/VisibilityOff"));
var _SettingsCard = _interopRequireDefault(require("./SettingsCard"));
var _FieldRow = _interopRequireDefault(require("./FieldRow"));
var _constants = require("./constants");
var _jsxRuntime = require("react/jsx-runtime");
function _interopRequireDefault(e) { return e && e.__esModule ? e : { "default": e }; }
function _interopRequireWildcard(e, t) { if ("function" == typeof WeakMap) var r = new WeakMap(), n = new WeakMap(); return (_interopRequireWildcard = function _interopRequireWildcard(e, t) { if (!t && e && e.__esModule) return e; var o, i, f = { __proto__: null, "default": e }; if (null === e || "object" != _typeof(e) && "function" != typeof e) return f; if (o = t ? n : r) { if (o.has(e)) return o.get(e); o.set(e, f); } for (var _t in e) "default" !== _t && {}.hasOwnProperty.call(e, _t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, _t)) && (i.get || i.set) ? o(f, _t, i) : f[_t] = e[_t]); return f; })(e, t); }
function ownKeys(e, r) { var t = Object.keys(e); if (Object.getOwnPropertySymbols) { var o = Object.getOwnPropertySymbols(e); r && (o = o.filter(function (r) { return Object.getOwnPropertyDescriptor(e, r).enumerable; })), t.push.apply(t, o); } return t; }
function _objectSpread(e) { for (var r = 1; r < arguments.length; r++) { var t = null != arguments[r] ? arguments[r] : {}; r % 2 ? ownKeys(Object(t), !0).forEach(function (r) { _defineProperty(e, r, t[r]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : ownKeys(Object(t)).forEach(function (r) { Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r)); }); } return e; }
function _defineProperty(e, r, t) { return (r = _toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function _toPropertyKey(t) { var i = _toPrimitive(t, "string"); return "symbol" == _typeof(i) ? i : i + ""; }
function _toPrimitive(t, r) { if ("object" != _typeof(t) || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != _typeof(i)) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }
function _regenerator() { /*! regenerator-runtime -- Copyright (c) 2014-present, Facebook, Inc. -- license (MIT): https://github.com/babel/babel/blob/main/packages/babel-helpers/LICENSE */ var e, t, r = "function" == typeof Symbol ? Symbol : {}, n = r.iterator || "@@iterator", o = r.toStringTag || "@@toStringTag"; function i(r, n, o, i) { var c = n && n.prototype instanceof Generator ? n : Generator, u = Object.create(c.prototype); return _regeneratorDefine2(u, "_invoke", function (r, n, o) { var i, c, u, f = 0, p = o || [], y = !1, G = { p: 0, n: 0, v: e, a: d, f: d.bind(e, 4), d: function d(t, r) { return i = t, c = 0, u = e, G.n = r, a; } }; function d(r, n) { for (c = r, u = n, t = 0; !y && f && !o && t < p.length; t++) { var o, i = p[t], d = G.p, l = i[2]; r > 3 ? (o = l === n) && (u = i[(c = i[4]) ? 5 : (c = 3, 3)], i[4] = i[5] = e) : i[0] <= d && ((o = r < 2 && d < i[1]) ? (c = 0, G.v = n, G.n = i[1]) : d < l && (o = r < 3 || i[0] > n || n > l) && (i[4] = r, i[5] = n, G.n = l, c = 0)); } if (o || r > 1) return a; throw y = !0, n; } return function (o, p, l) { if (f > 1) throw TypeError("Generator is already running"); for (y && 1 === p && d(p, l), c = p, u = l; (t = c < 2 ? e : u) || !y;) { i || (c ? c < 3 ? (c > 1 && (G.n = -1), d(c, u)) : G.n = u : G.v = u); try { if (f = 2, i) { if (c || (o = "next"), t = i[o]) { if (!(t = t.call(i, u))) throw TypeError("iterator result is not an object"); if (!t.done) return t; u = t.value, c < 2 && (c = 0); } else 1 === c && (t = i["return"]) && t.call(i), c < 2 && (u = TypeError("The iterator does not provide a '" + o + "' method"), c = 1); i = e; } else if ((t = (y = G.n < 0) ? u : r.call(n, G)) !== a) break; } catch (t) { i = e, c = 1, u = t; } finally { f = 1; } } return { value: t, done: y }; }; }(r, o, i), !0), u; } var a = {}; function Generator() {} function GeneratorFunction() {} function GeneratorFunctionPrototype() {} t = Object.getPrototypeOf; var c = [][n] ? t(t([][n]())) : (_regeneratorDefine2(t = {}, n, function () { return this; }), t), u = GeneratorFunctionPrototype.prototype = Generator.prototype = Object.create(c); function f(e) { return Object.setPrototypeOf ? Object.setPrototypeOf(e, GeneratorFunctionPrototype) : (e.__proto__ = GeneratorFunctionPrototype, _regeneratorDefine2(e, o, "GeneratorFunction")), e.prototype = Object.create(u), e; } return GeneratorFunction.prototype = GeneratorFunctionPrototype, _regeneratorDefine2(u, "constructor", GeneratorFunctionPrototype), _regeneratorDefine2(GeneratorFunctionPrototype, "constructor", GeneratorFunction), GeneratorFunction.displayName = "GeneratorFunction", _regeneratorDefine2(GeneratorFunctionPrototype, o, "GeneratorFunction"), _regeneratorDefine2(u), _regeneratorDefine2(u, o, "Generator"), _regeneratorDefine2(u, n, function () { return this; }), _regeneratorDefine2(u, "toString", function () { return "[object Generator]"; }), (_regenerator = function _regenerator() { return { w: i, m: f }; })(); }
function _regeneratorDefine2(e, r, n, t) { var i = Object.defineProperty; try { i({}, "", {}); } catch (e) { i = 0; } _regeneratorDefine2 = function _regeneratorDefine(e, r, n, t) { function o(r, n) { _regeneratorDefine2(e, r, function (e) { return this._invoke(r, n, e); }); } r ? i ? i(e, r, { value: n, enumerable: !t, configurable: !t, writable: !t }) : e[r] = n : (o("next", 0), o("throw", 1), o("return", 2)); }, _regeneratorDefine2(e, r, n, t); }
function asyncGeneratorStep(n, t, e, r, o, a, c) { try { var i = n[a](c), u = i.value; } catch (n) { return void e(n); } i.done ? t(u) : Promise.resolve(u).then(r, o); }
function _asyncToGenerator(n) { return function () { var t = this, e = arguments; return new Promise(function (r, o) { var a = n.apply(t, e); function _next(n) { asyncGeneratorStep(a, r, o, _next, _throw, "next", n); } function _throw(n) { asyncGeneratorStep(a, r, o, _next, _throw, "throw", n); } _next(void 0); }); }; }
function _slicedToArray(r, e) { return _arrayWithHoles(r) || _iterableToArrayLimit(r, e) || _unsupportedIterableToArray(r, e) || _nonIterableRest(); }
function _nonIterableRest() { throw new TypeError("Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); }
function _unsupportedIterableToArray(r, a) { if (r) { if ("string" == typeof r) return _arrayLikeToArray(r, a); var t = {}.toString.call(r).slice(8, -1); return "Object" === t && r.constructor && (t = r.constructor.name), "Map" === t || "Set" === t ? Array.from(r) : "Arguments" === t || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(t) ? _arrayLikeToArray(r, a) : void 0; } }
function _arrayLikeToArray(r, a) { (null == a || a > r.length) && (a = r.length); for (var e = 0, n = Array(a); e < a; e++) n[e] = r[e]; return n; }
function _iterableToArrayLimit(r, l) { var t = null == r ? null : "undefined" != typeof Symbol && r[Symbol.iterator] || r["@@iterator"]; if (null != t) { var e, n, i, u, a = [], f = !0, o = !1; try { if (i = (t = t.call(r)).next, 0 === l) { if (Object(t) !== t) return; f = !1; } else for (; !(f = (e = i.call(t)).done) && (a.push(e.value), a.length !== l); f = !0); } catch (r) { o = !0, n = r; } finally { try { if (!f && null != t["return"] && (u = t["return"](), Object(u) !== u)) return; } finally { if (o) throw n; } } return a; } }
function _arrayWithHoles(r) { if (Array.isArray(r)) return r; } // CRM KIT — PersonalDetailsForm: premium personal details card
var textFieldSx = {
  '& .MuiOutlinedInput-root': {
    borderRadius: "".concat(_constants.PA_RADIUS.md, "px"),
    fontFamily: _constants.PA_TYPOGRAPHY.fontFamily,
    fontSize: '0.875rem',
    backgroundColor: '#fff',
    transition: _constants.PA_TRANSITIONS.fast,
    '& .MuiOutlinedInput-notchedOutline': {
      borderColor: _constants.PA_COLORS.cardBorder,
      transition: _constants.PA_TRANSITIONS.fast
    },
    '&:hover .MuiOutlinedInput-notchedOutline': {
      borderColor: _constants.PA_COLORS.textMuted
    },
    '&.Mui-focused .MuiOutlinedInput-notchedOutline': {
      borderColor: _constants.PA_COLORS.accent,
      borderWidth: '1.5px'
    },
    '&.Mui-focused': {
      boxShadow: "0 0 0 3px ".concat(_constants.PA_COLORS.accentLight)
    },
    '&.Mui-disabled': {
      backgroundColor: _constants.PA_COLORS.sectionBg,
      opacity: 0.7
    }
  },
  '& .MuiInputLabel-root': {
    fontFamily: _constants.PA_TYPOGRAPHY.fontFamily,
    fontSize: '0.8125rem'
  }
};
function PersonalDetailsForm(_ref) {
  var user = _ref.user,
    onSaveProfile = _ref.onSaveProfile,
    onChangePassword = _ref.onChangePassword,
    _ref$saving = _ref.saving,
    saving = _ref$saving === void 0 ? false : _ref$saving;
  var _useState = (0, _react.useState)(false),
    _useState2 = _slicedToArray(_useState, 2),
    isEditing = _useState2[0],
    setIsEditing = _useState2[1];
  var _useState3 = (0, _react.useState)({}),
    _useState4 = _slicedToArray(_useState3, 2),
    edited = _useState4[0],
    setEdited = _useState4[1];
  var _useState5 = (0, _react.useState)(false),
    _useState6 = _slicedToArray(_useState5, 2),
    showPasswordSection = _useState6[0],
    setShowPasswordSection = _useState6[1];
  var _useState7 = (0, _react.useState)(''),
    _useState8 = _slicedToArray(_useState7, 2),
    oldPassword = _useState8[0],
    setOldPassword = _useState8[1];
  var _useState9 = (0, _react.useState)(''),
    _useState0 = _slicedToArray(_useState9, 2),
    newPassword = _useState0[0],
    setNewPassword = _useState0[1];
  var _useState1 = (0, _react.useState)(''),
    _useState10 = _slicedToArray(_useState1, 2),
    confirmPassword = _useState10[0],
    setConfirmPassword = _useState10[1];
  var _useState11 = (0, _react.useState)(false),
    _useState12 = _slicedToArray(_useState11, 2),
    showOld = _useState12[0],
    setShowOld = _useState12[1];
  var _useState13 = (0, _react.useState)(false),
    _useState14 = _slicedToArray(_useState13, 2),
    showNew = _useState14[0],
    setShowNew = _useState14[1];
  var _useState15 = (0, _react.useState)(false),
    _useState16 = _slicedToArray(_useState15, 2),
    showConfirm = _useState16[0],
    setShowConfirm = _useState16[1];
  var _useState17 = (0, _react.useState)(false),
    _useState18 = _slicedToArray(_useState17, 2),
    passwordSaving = _useState18[0],
    setPasswordSaving = _useState18[1];
  var _useState19 = (0, _react.useState)(null),
    _useState20 = _slicedToArray(_useState19, 2),
    feedback = _useState20[0],
    setFeedback = _useState20[1]; // { type: 'success'|'error', message }

  (0, _react.useEffect)(function () {
    if (user) {
      setEdited({
        username: user.username || '',
        firstName: user.firstName || '',
        lastName: user.lastName || '',
        email: user.email || '',
        phone: user.phone || ''
      });
    }
  }, [user]);
  var handleSave = /*#__PURE__*/function () {
    var _ref2 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee() {
      var result;
      return _regenerator().w(function (_context) {
        while (1) switch (_context.n) {
          case 0:
            if (onSaveProfile) {
              _context.n = 1;
              break;
            }
            return _context.a(2);
          case 1:
            _context.n = 2;
            return onSaveProfile(edited);
          case 2:
            result = _context.v;
            if (result !== null && result !== void 0 && result.success) {
              setIsEditing(false);
              setFeedback({
                type: 'success',
                message: 'הפרטים נשמרו בהצלחה'
              });
            } else {
              setFeedback({
                type: 'error',
                message: (result === null || result === void 0 ? void 0 : result.message) || 'שגיאה בשמירה'
              });
            }
            setTimeout(function () {
              return setFeedback(null);
            }, 4000);
          case 3:
            return _context.a(2);
        }
      }, _callee);
    }));
    return function handleSave() {
      return _ref2.apply(this, arguments);
    };
  }();
  var handleCancel = function handleCancel() {
    setIsEditing(false);
    setEdited({
      username: user.username || '',
      firstName: user.firstName || '',
      lastName: user.lastName || '',
      email: user.email || '',
      phone: user.phone || ''
    });
  };
  var handleChangePassword = /*#__PURE__*/function () {
    var _ref3 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee2() {
      var result;
      return _regenerator().w(function (_context2) {
        while (1) switch (_context2.n) {
          case 0:
            if (!(newPassword !== confirmPassword)) {
              _context2.n = 1;
              break;
            }
            setFeedback({
              type: 'error',
              message: 'הסיסמאות אינן תואמות'
            });
            setTimeout(function () {
              return setFeedback(null);
            }, 4000);
            return _context2.a(2);
          case 1:
            if (!(newPassword.length < 4)) {
              _context2.n = 2;
              break;
            }
            setFeedback({
              type: 'error',
              message: 'סיסמה חדשה חייבת להכיל לפחות 4 תווים'
            });
            setTimeout(function () {
              return setFeedback(null);
            }, 4000);
            return _context2.a(2);
          case 2:
            if (onChangePassword) {
              _context2.n = 3;
              break;
            }
            return _context2.a(2);
          case 3:
            setPasswordSaving(true);
            _context2.n = 4;
            return onChangePassword(oldPassword, newPassword);
          case 4:
            result = _context2.v;
            setPasswordSaving(false);
            if (result !== null && result !== void 0 && result.success) {
              setShowPasswordSection(false);
              setOldPassword('');
              setNewPassword('');
              setConfirmPassword('');
              setFeedback({
                type: 'success',
                message: 'הסיסמה שונתה בהצלחה'
              });
            } else {
              setFeedback({
                type: 'error',
                message: (result === null || result === void 0 ? void 0 : result.message) || 'שגיאה בשינוי סיסמה'
              });
            }
            setTimeout(function () {
              return setFeedback(null);
            }, 4000);
          case 5:
            return _context2.a(2);
        }
      }, _callee2);
    }));
    return function handleChangePassword() {
      return _ref3.apply(this, arguments);
    };
  }();
  var passwordToggle = function passwordToggle(show, setShow) {
    return /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.InputAdornment, {
      position: "end",
      children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.IconButton, {
        size: "small",
        onClick: function onClick() {
          return setShow(!show);
        },
        edge: "end",
        children: show ? /*#__PURE__*/(0, _jsxRuntime.jsx)(_VisibilityOff["default"], {
          fontSize: "small"
        }) : /*#__PURE__*/(0, _jsxRuntime.jsx)(_Visibility["default"], {
          fontSize: "small"
        })
      })
    });
  };
  return /*#__PURE__*/(0, _jsxRuntime.jsxs)(_SettingsCard["default"], {
    title: "\u05E4\u05E8\u05D8\u05D9\u05DD \u05D0\u05D9\u05E9\u05D9\u05D9\u05DD",
    description: "\u05E0\u05D9\u05D4\u05D5\u05DC \u05E4\u05E8\u05D8\u05D9 \u05D4\u05D7\u05E9\u05D1\u05D5\u05DF \u05D5\u05E9\u05D9\u05E0\u05D5\u05D9 \u05E1\u05D9\u05E1\u05DE\u05D4",
    icon: /*#__PURE__*/(0, _jsxRuntime.jsx)(_PersonRounded["default"], {}),
    children: [/*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Collapse, {
      "in": !!feedback,
      children: feedback && /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Alert, {
        severity: feedback.type,
        onClose: function onClose() {
          return setFeedback(null);
        },
        sx: {
          borderRadius: "".concat(_constants.PA_RADIUS.sm, "px"),
          fontFamily: _constants.PA_TYPOGRAPHY.fontFamily,
          fontSize: '0.8125rem'
        },
        children: feedback.message
      })
    }), /*#__PURE__*/(0, _jsxRuntime.jsxs)(_material.Box, {
      sx: {
        display: 'grid',
        gridTemplateColumns: {
          xs: '1fr',
          sm: '1fr 1fr'
        },
        gap: '20px'
      },
      children: [/*#__PURE__*/(0, _jsxRuntime.jsx)(_FieldRow["default"], {
        label: "\u05E9\u05DD \u05E4\u05E8\u05D8\u05D9",
        children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.TextField, {
          size: "small",
          fullWidth: true,
          value: edited.firstName || '',
          onChange: function onChange(e) {
            return setEdited(_objectSpread(_objectSpread({}, edited), {}, {
              firstName: e.target.value
            }));
          },
          disabled: !isEditing,
          placeholder: "\u05D4\u05D6\u05DF \u05E9\u05DD \u05E4\u05E8\u05D8\u05D9",
          sx: textFieldSx
        })
      }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_FieldRow["default"], {
        label: "\u05E9\u05DD \u05DE\u05E9\u05E4\u05D7\u05D4",
        children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.TextField, {
          size: "small",
          fullWidth: true,
          value: edited.lastName || '',
          onChange: function onChange(e) {
            return setEdited(_objectSpread(_objectSpread({}, edited), {}, {
              lastName: e.target.value
            }));
          },
          disabled: !isEditing,
          placeholder: "\u05D4\u05D6\u05DF \u05E9\u05DD \u05DE\u05E9\u05E4\u05D7\u05D4",
          sx: textFieldSx
        })
      }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_FieldRow["default"], {
        label: "\u05E9\u05DD \u05DE\u05E9\u05EA\u05DE\u05E9",
        children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.TextField, {
          size: "small",
          fullWidth: true,
          value: edited.username || '',
          onChange: function onChange(e) {
            return setEdited(_objectSpread(_objectSpread({}, edited), {}, {
              username: e.target.value
            }));
          },
          disabled: !isEditing,
          placeholder: "\u05D4\u05D6\u05DF \u05E9\u05DD \u05DE\u05E9\u05EA\u05DE\u05E9",
          sx: textFieldSx
        })
      }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_FieldRow["default"], {
        label: "\u05D0\u05D9\u05DE\u05D9\u05D9\u05DC",
        children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.TextField, {
          size: "small",
          fullWidth: true,
          type: "email",
          value: edited.email || '',
          onChange: function onChange(e) {
            return setEdited(_objectSpread(_objectSpread({}, edited), {}, {
              email: e.target.value
            }));
          },
          disabled: !isEditing,
          placeholder: "example@email.com",
          sx: textFieldSx
        })
      }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_FieldRow["default"], {
        label: "\u05D8\u05DC\u05E4\u05D5\u05DF",
        sx: {
          gridColumn: {
            sm: '1 / -1'
          },
          maxWidth: {
            sm: 'calc(50% - 12px)'
          }
        },
        children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.TextField, {
          size: "small",
          fullWidth: true,
          value: edited.phone || '',
          onChange: function onChange(e) {
            return setEdited(_objectSpread(_objectSpread({}, edited), {}, {
              phone: e.target.value
            }));
          },
          disabled: !isEditing,
          placeholder: "050-0000000",
          sx: textFieldSx
        })
      })]
    }), /*#__PURE__*/(0, _jsxRuntime.jsxs)(_material.Box, {
      sx: {
        display: 'flex',
        gap: '10px',
        flexWrap: 'wrap',
        alignItems: 'center'
      },
      children: [!isEditing ? /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Button, {
        variant: "outlined",
        startIcon: /*#__PURE__*/(0, _jsxRuntime.jsx)(_EditRounded["default"], {
          sx: {
            fontSize: 18
          }
        }),
        onClick: function onClick() {
          return setIsEditing(true);
        },
        sx: _objectSpread(_objectSpread({
          borderRadius: "".concat(_constants.PA_RADIUS.sm, "px"),
          fontFamily: _constants.PA_TYPOGRAPHY.fontFamily
        }, _constants.PA_TYPOGRAPHY.buttonText), {}, {
          textTransform: 'none',
          borderColor: _constants.PA_COLORS.cardBorder,
          color: _constants.PA_COLORS.textPrimary,
          padding: '7px 18px',
          transition: _constants.PA_TRANSITIONS.fast,
          '&:hover': {
            borderColor: _constants.PA_COLORS.accent,
            color: _constants.PA_COLORS.accent,
            backgroundColor: _constants.PA_COLORS.accentLight
          }
        }),
        children: "\u05E2\u05E8\u05D9\u05DB\u05D4"
      }) : /*#__PURE__*/(0, _jsxRuntime.jsxs)(_jsxRuntime.Fragment, {
        children: [/*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Button, {
          variant: "contained",
          startIcon: saving ? /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.CircularProgress, {
            size: 16,
            color: "inherit"
          }) : /*#__PURE__*/(0, _jsxRuntime.jsx)(_CheckRounded["default"], {
            sx: {
              fontSize: 18
            }
          }),
          onClick: handleSave,
          disabled: saving,
          sx: _objectSpread(_objectSpread({
            borderRadius: "".concat(_constants.PA_RADIUS.sm, "px"),
            fontFamily: _constants.PA_TYPOGRAPHY.fontFamily
          }, _constants.PA_TYPOGRAPHY.buttonText), {}, {
            textTransform: 'none',
            backgroundColor: _constants.PA_COLORS.accent,
            padding: '7px 22px',
            boxShadow: 'none',
            transition: _constants.PA_TRANSITIONS.fast,
            '&:hover': {
              backgroundColor: _constants.PA_COLORS.accentHover,
              boxShadow: 'none'
            }
          }),
          children: "\u05E9\u05DE\u05D9\u05E8\u05D4"
        }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Button, {
          variant: "text",
          startIcon: /*#__PURE__*/(0, _jsxRuntime.jsx)(_CloseRounded["default"], {
            sx: {
              fontSize: 18
            }
          }),
          onClick: handleCancel,
          sx: _objectSpread(_objectSpread({
            borderRadius: "".concat(_constants.PA_RADIUS.sm, "px"),
            fontFamily: _constants.PA_TYPOGRAPHY.fontFamily
          }, _constants.PA_TYPOGRAPHY.buttonText), {}, {
            textTransform: 'none',
            color: _constants.PA_COLORS.textSecondary,
            padding: '7px 14px',
            transition: _constants.PA_TRANSITIONS.fast,
            '&:hover': {
              backgroundColor: _constants.PA_COLORS.sectionBg
            }
          }),
          children: "\u05D1\u05D9\u05D8\u05D5\u05DC"
        })]
      }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Box, {
        sx: {
          flex: 1
        }
      }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Button, {
        variant: "text",
        startIcon: /*#__PURE__*/(0, _jsxRuntime.jsx)(_LockRounded["default"], {
          sx: {
            fontSize: 18
          }
        }),
        onClick: function onClick() {
          return setShowPasswordSection(!showPasswordSection);
        },
        sx: _objectSpread(_objectSpread({
          borderRadius: "".concat(_constants.PA_RADIUS.sm, "px"),
          fontFamily: _constants.PA_TYPOGRAPHY.fontFamily
        }, _constants.PA_TYPOGRAPHY.buttonText), {}, {
          textTransform: 'none',
          color: _constants.PA_COLORS.textMuted,
          padding: '7px 14px',
          transition: _constants.PA_TRANSITIONS.fast,
          '&:hover': {
            backgroundColor: _constants.PA_COLORS.sectionBg,
            color: _constants.PA_COLORS.textSecondary
          }
        }),
        children: showPasswordSection ? 'ביטול' : 'שינוי סיסמה'
      })]
    }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Collapse, {
      "in": showPasswordSection,
      children: /*#__PURE__*/(0, _jsxRuntime.jsxs)(_material.Box, {
        sx: {
          display: 'flex',
          flexDirection: 'column',
          gap: '20px',
          padding: '20px',
          backgroundColor: _constants.PA_COLORS.sectionBg,
          borderRadius: "".concat(_constants.PA_RADIUS.md, "px"),
          border: "1px solid ".concat(_constants.PA_COLORS.divider)
        },
        children: [/*#__PURE__*/(0, _jsxRuntime.jsxs)(_material.Box, {
          sx: {
            display: 'flex',
            alignItems: 'center',
            gap: '8px'
          },
          children: [/*#__PURE__*/(0, _jsxRuntime.jsx)(_LockRounded["default"], {
            sx: {
              fontSize: 16,
              color: _constants.PA_COLORS.textMuted
            }
          }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Typography, {
            sx: _objectSpread(_objectSpread({}, _constants.PA_TYPOGRAPHY.fieldLabel), {}, {
              fontFamily: _constants.PA_TYPOGRAPHY.fontFamily,
              color: _constants.PA_COLORS.textPrimary,
              fontWeight: 600
            }),
            children: "\u05E9\u05D9\u05E0\u05D5\u05D9 \u05E1\u05D9\u05E1\u05DE\u05D4"
          })]
        }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_FieldRow["default"], {
          label: "\u05E1\u05D9\u05E1\u05DE\u05D4 \u05E0\u05D5\u05DB\u05D7\u05D9\u05EA",
          children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.TextField, {
            size: "small",
            fullWidth: true,
            type: showOld ? 'text' : 'password',
            value: oldPassword,
            onChange: function onChange(e) {
              return setOldPassword(e.target.value);
            },
            placeholder: "\u05D4\u05D6\u05DF \u05E1\u05D9\u05E1\u05DE\u05D4 \u05E0\u05D5\u05DB\u05D7\u05D9\u05EA",
            InputProps: {
              endAdornment: passwordToggle(showOld, setShowOld)
            },
            sx: textFieldSx
          })
        }), /*#__PURE__*/(0, _jsxRuntime.jsxs)(_material.Box, {
          sx: {
            display: 'grid',
            gridTemplateColumns: {
              xs: '1fr',
              sm: '1fr 1fr'
            },
            gap: '20px'
          },
          children: [/*#__PURE__*/(0, _jsxRuntime.jsx)(_FieldRow["default"], {
            label: "\u05E1\u05D9\u05E1\u05DE\u05D4 \u05D7\u05D3\u05E9\u05D4",
            children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.TextField, {
              size: "small",
              fullWidth: true,
              type: showNew ? 'text' : 'password',
              value: newPassword,
              onChange: function onChange(e) {
                return setNewPassword(e.target.value);
              },
              placeholder: "\u05DC\u05E4\u05D7\u05D5\u05EA 4 \u05EA\u05D5\u05D5\u05D9\u05DD",
              InputProps: {
                endAdornment: passwordToggle(showNew, setShowNew)
              },
              sx: textFieldSx
            })
          }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_FieldRow["default"], {
            label: "\u05D0\u05D9\u05E9\u05D5\u05E8 \u05E1\u05D9\u05E1\u05DE\u05D4 \u05D7\u05D3\u05E9\u05D4",
            children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.TextField, {
              size: "small",
              fullWidth: true,
              type: showConfirm ? 'text' : 'password',
              value: confirmPassword,
              onChange: function onChange(e) {
                return setConfirmPassword(e.target.value);
              },
              placeholder: "\u05D4\u05D6\u05DF \u05E9\u05D5\u05D1 \u05D0\u05EA \u05D4\u05E1\u05D9\u05E1\u05DE\u05D4",
              error: confirmPassword.length > 0 && newPassword !== confirmPassword,
              helperText: confirmPassword.length > 0 && newPassword !== confirmPassword ? 'הסיסמאות אינן תואמות' : '',
              InputProps: {
                endAdornment: passwordToggle(showConfirm, setShowConfirm)
              },
              sx: textFieldSx
            })
          })]
        }), /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Box, {
          children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.Button, {
            variant: "contained",
            onClick: handleChangePassword,
            disabled: passwordSaving || !oldPassword || !newPassword || !confirmPassword,
            startIcon: passwordSaving ? /*#__PURE__*/(0, _jsxRuntime.jsx)(_material.CircularProgress, {
              size: 16,
              color: "inherit"
            }) : /*#__PURE__*/(0, _jsxRuntime.jsx)(_LockRounded["default"], {
              sx: {
                fontSize: 18
              }
            }),
            sx: _objectSpread(_objectSpread({
              borderRadius: "".concat(_constants.PA_RADIUS.sm, "px"),
              fontFamily: _constants.PA_TYPOGRAPHY.fontFamily
            }, _constants.PA_TYPOGRAPHY.buttonText), {}, {
              textTransform: 'none',
              backgroundColor: _constants.PA_COLORS.accent,
              padding: '8px 24px',
              boxShadow: 'none',
              transition: _constants.PA_TRANSITIONS.fast,
              '&:hover': {
                backgroundColor: _constants.PA_COLORS.accentHover,
                boxShadow: 'none'
              }
            }),
            children: "\u05E2\u05D3\u05DB\u05D5\u05DF \u05E1\u05D9\u05E1\u05DE\u05D4"
          })
        })]
      })
    })]
  });
}
//# sourceMappingURL=PersonalDetailsForm.js.map